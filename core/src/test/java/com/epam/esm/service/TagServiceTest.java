...
public class TagServiceTest {
   ...
   @Test
   void getAll(){
       Mockito.when(tagRepository.readAll(0, 0)).thenReturn(tags);
       Assertions.assertEquals(tags,tagRepository.readAll(0,0));
   }

   @Test
   void getByIDExisting(){
       Mockito.when(tagRepository.findByID(Mockito.anyLong())).thenReturn(tag);
